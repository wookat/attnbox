...
| Dimension | Status (round 205 / v0.4.8 released) | Evidence |
|---|---|---|
...
| Security posture | ✅ SECURITY.md; localhost-only by default, `--host` gated by mandatory bearer token on every API/SSE surface (negative-tested rounds 29/84, re-proven rounds 163/182/202); read-only collectors; hook inputs validated; `/api/ack` input-hardened | `SECURITY.md`, `docs/gap/GAP-ROUND-84.md` |
...
| Mobile-first UI | ✅ responsive inbox, PWA install, offline last-known snapshot, actionable browser notifications, WCAG AA both themes (axe 0 violations across 10 interactive states, re-audited rounds 147/162/178/196), inbox Lighthouse a11y 100 | `apps/web`, `docs/gap/GAP-ROUND-147.md` |
| Performance at scale | ✅ live ~3,200 sessions: full Devin backlog crawl (cap 10,000), slim SSE (done sessions lazily loaded) — mobile Lighthouse perf 94 across all runs / TBT ≤10 ms at ~3,214 sessions (round-201), SSE gzip, daemon RSS flat ~130 MB (15-min soak @3,226, round-203) | `docs/gap/GAP-ROUND-201.md`, `docs/gap/GAP-ROUND-203.md` |
| Real-world validation | ✅ continuous dogfood against a live 3,200-session org: waiting/reply/ack/webhook paths end-to-end probed (controlled Devin probes rounds 93/137), hooks installer negative-tested (rounds 99/132/164/181/193), notification/webhook storm guards live-fire proven on real transitions (rounds 157/163/182/187/202 — real transitions each notified exactly once, stock never re-notified), all three local collectors live-fire verified (rounds 161/177/188/199), offline snapshot + SSE reconnect re-proven on the slim build (rounds 171/195) | `docs/gap/GAP-ROUND-157.md`, `docs/gap/GAP-ROUND-161.md` |
