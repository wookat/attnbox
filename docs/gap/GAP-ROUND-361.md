# GAP-ROUND-361 — 分诊全流程 UX 走查（纯文档）

Round 361. Driver dimension: full triage-flow UX re-walk
(search → filter → ack all → un-ack + keyboard chain), first
since round-349. Real Chrome/CDP against a live probe daemon
(~3,68x sessions). Probe cleared localStorage before measuring
(round-356 method note).
...
✓ all done:          34 items acked in one click
