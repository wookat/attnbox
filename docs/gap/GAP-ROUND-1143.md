# GAP-ROUND-1143：本地采集器实弹抽查（round-1132 后首次）

日期：2026-08-04 ｜ 基线：main edf4c42（#1177 合并后，99 测试绿）｜ live 面：5,532 会话（迄今最大）

## 结论

1. **无 P0/P1。** 三采集器沙箱 fixture 14/14 首跑全对，零假 FAIL。
2. 沙箱（temp HOME）零残留，主 daemon 4820 不受影响（total=5,532，items==summary.total 5532==5532 成立）。
...
- 探针本机 `~/a11y/coll1143.tmp.mjs`（round-1132 探针复制件，不入库，轮后已删）：temp HOME 沙箱构造 Claude/Codex/Gemini fixture 后直调各采集器 dist `collect()`，dist 为 #1177 合并后 `pnpm build`（EXIT=0）全新构建产物。
- 沙箱前缀 `attnbox1077-*`：轮后 `/tmp` 精确核验 0 残留（`ls /tmp | grep attnbox1077` 空）。
...
rounds 1133–1142 合并面（全为纯文档轮）无采集器回归。三源被动聚合差异化不变，继续循环。
