# GAP-ROUND-1025: CLI 黄金路径复走（round-1014 后首次）

日期：2026-08-17。基线：main `7e8e77f`（#1059 合并后）。结论先行：**doctor / ls --waiting / hooks --install 四态全部首跑全通，无 P0/P1。**
...
### ls --waiting（热跑 ~4.7s @5,429 会话，迄今最大）

- 10 waiting 全带预览/时长/行动链接（session URL + PR 次级链接）。
- 尾行计数与 API 同刻精确一致：10/45/5429 == API waiting=10 working=45 total=5429（本轮无观察竞态）。
...
沙箱零残留。rounds 1015–1024 合并面无 CLI 回归。
