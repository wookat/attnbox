# GAP-ROUND-967：本地采集器实弹抽查（round-956 后首次）

日期：2026-08-04。结论先行：**三采集器沙箱 fixture 14/14 首跑全对，零假 FAIL，无 P0/P1，纯文档轮。**

## 环境

- main @ #1001（ROUND-966 分诊走查）合并后回归面；本地门禁全绿（lint ✓ / typecheck ✓ / build ✓ / test 99 ✓）。
- 实机 dogfood daemon（127.0.0.1:4820）@ 5,339 会话（迄今最大）、13 waiting、48 working。
- 探针：沙箱 fixture（隔离 HOME，不触真实转录），方法沿用 rounds 934/945/956。
...
- rounds 957–966 合并面对三采集器状态判定无回归；
- 零假 FAIL，沙箱零残留；
